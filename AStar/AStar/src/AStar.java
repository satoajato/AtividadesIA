...
public class AStar<T> {
    ...
    public void buscar(Vertex<T> raiz) {
        ...
        while(!aberta.isEmpty()) {
            ...
            for (Vertex<T> filho : filhos) {
                System.out.println("Filho: " + filho.getDado());
    ...
    private int Dijkstra(Vertex<T> inicio, Vertex<T> objetivo) { //algoritmo pra calcular menor distancia (noinicial = u e objt = v)
        ...
        qDij.enqueue(atual,0,atual.getDji());
        System.out.println("\nCria pilha de Dijkstra");
        
        while(!qDij.isEmpty()) { //se a pilha estiver vazia
            atual = qDij.dequeue();
            ...
            if (atual.equals(objetivo)) {
                ...
                while(atual.getPaiDji() !=null) {
                    distancia += atual.getDji();
                    atual = atual.getPaiDji();
                }
                return distancia;
            }
        }
        System.out.println("Não existe caminho de " + inicio.getDado() + " até " + objetivo.getDado());
        return distancia;
    }
}